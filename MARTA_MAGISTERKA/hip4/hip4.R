...
suple <- dane_excel %>%
  filter(
    `13. Czy przyjmuje Pan_i suplement, który w swoim składzie zawiera sylimarynę (ostropest plamisty)?`=='Tak')%>%
  select(starts_with("Jak często w ciągu ostatnich")) %>%
  koduj_wartosci()

NieSuple <- dane_excel %>%
  filter(
    `13. Czy przyjmuje Pan_i suplement, który w swoim składzie zawiera sylimarynę (ostropest plamisty)?`=='Nie')%>%
  select(starts_with("Jak często w ciągu ostatnich")) %>%
  koduj_wartosci()
...
tablica_danych_suple <- tworz_tablice_danych(suple)
tablica_danych_nieSuple <- tworz_tablice_danych(NieSuple)
...
for (kategoria in kategorie) {
  ...
  suma_nieSuple <- tablica_danych_nieSuple[[kolumna]]
  suma_suple <- tablica_danych_suple[[kolumna]]
  ...
  dane_do_wykresu_nieSuple <- data.frame(
    Kategoria = c("Liczba osób", "Średnia ilość punktów", "Maksymalna ilość punktów"),
    Wartość = c(length(suma_nieSuple), mean(suma_nieSuple, na.rm = TRUE), maksymalna_mozliwa_ilosc_punktow),
    Grupa = "Osoby niesuplementujące"
  )
  ...
  dane_do_wykresu_suple <- data.frame(
    Kategoria = c("Liczba osób", "Średnia ilość punktów", "Maksymalna ilość punktów"),
    Wartość = c(length(suma_suple), mean(suma_suple, na.rm = TRUE), maksymalna_mozliwa_ilosc_punktow),
    Grupa = "Osoby suplementujace"
  )
  ...
  dane_do_wykresu <- bind_rows(dane_do_wykresu_nieSuple,dane_do_wykresu_suple)
  #dane_do_wykresu <- bind_rows(dane_do_wykresu_nieWykrz, dane_do_wykresu_wykrz)
  
  # Obliczanie testu t dla niezależnych prób
  #test_t <- ks.test(suma_nieSuple, suma_suple)#tutaj 2 z 8 jest istotna
  #test_t <- kruskal.test(list(suma_nieWykrz, suma_wykrz))#kruskal wallis
  test_t <- kruskal.test(list(suma_nieSuple,suma_suple))
  #test_t <- wilcox.test(suma_nieSuple,suma_suple)
