...
an <- read_excel("Dane-i-hipotezy.xlsx")
...
# Dane dla wykresu
dane <- data.frame(
  Grupa = c("1-5 Miesięcy\n3-5 miesięcy", "6-10 miesięcy\nRok", "Ponad Rok"),
  Przed = c(srednia1przed, srednia2przed, srednia3przed),
  Po = c(srednia1po, srednia2po, srednia3po),
...
wykres <- ggplot(dane, aes(x = Grupa)) +
  ...
  geom_text(aes(y = Po, label = paste(Po)), position = position_dodge(width = 0.9), vjust = -0.5, color = "blue") +
  ...
  scale_fill_manual(values = c("Przed" = "green", "Po" = "red"))
