...
t_samoocena <- t.test(
  grupa_Ogladaczy$`21.  Czy cierpisz z powodu niskiej samooceny i/lub zaburzeń emocjonalnych ?_kodowane`,
  grupa_NieOgladaczy$`21.  Czy cierpisz z powodu niskiej samooceny i/lub zaburzeń emocjonalnych ?_kodowane`,
...
liczba_osob_zla_samoocena <- sum(grupa_Ogladaczy$`21.  Czy cierpisz z powodu niskiej samooceny i/lub zaburzeń emocjonalnych ?_kodowane` == 1)
...
dane_wykres <- data.frame(
  ...
  Zla_Samoocena = c(liczba_osob_zla_samoocena, sum(grupa_NieOgladaczy$`21.  Czy cierpisz z powodu niskiej samooceny i/lub zaburzeń emocjonalnych ?_kodowane` == 1)),
  Procent_Zla_Samoocena = c(procent_zla_samoocena, sum(grupa_NieOgladaczy$`21.  Czy cierpisz z powodu niskiej samooceny i/lub zaburzeń emocjonalnych ?_kodowane` == 1) / nrow(grupa_NieOgladaczy) * 100)
)
...
geom_bar(aes(y = Zla_Samoocena, fill = "Osoby z niską samooceną"), stat = "identity", width = 0.5) +
scale_fill_manual(values = c("Wszystkie Osoby" = "lightblue", "Osoby z niską samooceną" = "orange")) +
...
labs(title = "Liczba osób z złą samooceną w porównaniu z ogólną liczbą osób w grupach",
        x = "Grupa",
        y = "Liczba osób",
        fill = "Legenda") +
   theme_classic() +
