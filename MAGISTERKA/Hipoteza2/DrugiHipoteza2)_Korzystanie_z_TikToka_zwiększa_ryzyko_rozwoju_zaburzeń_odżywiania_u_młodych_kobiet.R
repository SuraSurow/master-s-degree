...
EAT26_testA <- read_excel("C:/Users/admin/Desktop/MAGISTERKA/Hipoteza2/eat26_testA/ankieta_kodowanieEAT26_testA.xlsx")
EAT26_testB <- read_excel("C:/Users/admin/Desktop/MAGISTERKA/Hipoteza2/eat26_testB/ankieta_kodowanieEAT26_testB.xlsx")
...
wyniki <- wplyw %>%
  left_join(select(EAT26_testA, ID, Interpretacja), by = "ID") %>%
  rename(InterpretacjaTestA = Interpretacja)


grupa_wpływ <- wyniki %>%
  filter(Czy_TikTok_wpływał == 1) %>%
  pull(InterpretacjaTestA)
...
dane_wpływ <- wyniki %>%
  ...
  summarise(
    Total = n(),
    With_Interpretacja = sum(InterpretacjaTestA == 1, na.rm = TRUE)
  )%>%
...
ggplot(dane_wpływ, aes(x = factor(Czy_TikTok_wpływał, levels = c(0, 1)), y = Total, fill = "Wszystkie osoby")) +
  ...
  labs(
    title = "Wpływ TikToka na ryzyko zaburzeń odżywiania EAT26-A",
    x = "Wpływ TikToka",
    y = "Liczba osób",
    fill = "Legenda"
  ) +
